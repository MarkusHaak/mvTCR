...
def parse_args():
    ...
    parser.add_argument('--continue', action='store_true', help='continue optuna study if it already exist instead of overwriting it. If <--runs> is greater than 1, for the consecutive runs, the studies are always continued.')
    args = parser.parse_args()
    for exp in args.experiments:
        if not os.path.exists(exp):
            print(f"ERROR: experiment {exp} does not exist")
            exit(1)
        elif os.patch.basename(exp) not in ['10x_optuna.py']:
...
def main():
    ...
    for exp in args.experiments:
        for model in args.models:
            for tcr_emb in args.tcr_embs:
                cmd_str = f"{PYTHONPATH} {exp} --model {model} --tcr_emb {tcr_emb} --timeout {args.timeout}"
                commands.append(cmd_str)
            if args.no_tcr_emb:
                cmd_str = f"{PYTHONPATH} {exp} --model {model} --timeout {args.timeout}"
                commands.append(cmd_str)
    ...
    for i in range(args.runs):
        for cmd_str in commands:
            if (i == 0 and args.continue) or i > 0:
                cmd_str += " --continue"
