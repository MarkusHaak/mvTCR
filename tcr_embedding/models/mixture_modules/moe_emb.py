...
class MoEEmbModelTorch(nn.Module):
	def __init__(self, tcr_params, rna_params, joint_params):
		super(MoEEmbModelTorch, self).__init__()
		...
		edim = tcr_params['max_tcr_length']
		...
		self.tcr_encoder = build_mlp_encoder(tcr_params, edim, hdim)
		self.tcr_decoder = build_mlp_decoder(tcr_params, edim, hdim)
	...
	def forward(self, rna, tcr, tcr_len, conditional=None):
		...
		for z_ in z:
			...
			tcr_pred.append(self.tcr_decoder(f_tcr))
...
class MoEEmbModel(VAEBaseModel):
	def __init__(self,
				 adata,
				 params_architecture,
		...
		super(MoEEmbModel, self).__init__(adata, params_architecture, balanced_sampling, metadata,
									   conditional, optimization_mode_params, label_key, device)
		self.model_type = 'moe_emb'

		self.params_tcr['max_tcr_length'] = adata.obsm[self.tcr_emb].shape[1] # self.tcr_emb = self.params_tcr['tcr_emb'] is set in VAEBaseModel
		...
		self.model = MoEEmbModelTorch(self.params_tcr, self.params_rna, self.params_joint)
	...
	def calculate_loss(self, rna_pred, rna, tcr_pred, tcr):
		...
		#	tcr_loss *= 0.5 * self.loss_weights[1]
		tcr_loss = self.loss_function_tcr(tcr_pred[0], tcr) + self.loss_function_tcr(tcr_pred[1], tcr)
		tcr_loss *= 0.5 * self.loss_weights[1]
	...
	def get_latent_unimodal(self, adata, metadata, modality, return_mean=True):
		...
		"""
		data_embed = initialize_prediction_loader(adata, metadata, self.batch_size, beta_only=self.beta_only, tcr_emb=self.tcr_emb)
